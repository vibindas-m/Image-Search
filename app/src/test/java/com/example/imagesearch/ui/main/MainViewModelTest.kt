...
class MainViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = MainViewModel(imageSearchUseCase)
    }
    ...
    fun `updateImageSearchResult test`() {
        ...
        Assert.assertEquals(null, viewModel.imageSearchList.value)
        viewModel.updateImageSearchResult(ImageSearchResultModel(totalCount = 0, imageSearchList = listOf()))
        Assert.assertEquals(emptyList<ImageSearchModel>(), viewModel.imageSearchList.value)

        val resultModel = ImageSearchResultModel(totalCount = 1, imageSearchList = listOf(ImageSearchModel(
    ...
    fun `get imageSearch test`() {
        ...
        every { imageSearchUseCase.execute() } returns useCaseResult
        ...
        verify { imageSearchUseCase.execute() }
    }
